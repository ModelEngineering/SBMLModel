...
class DClassEdit(rpickle.RPickler):
...
class TestRPickler(unittest.TestCase):
    ...
    def testRpEdit(self):
        if IGNORE_TEST:
            return
        obj = self.cls_edit(C_VALUE)
        self.assertFalse("a" in obj.__dict__.keys())
        obj.rpSerialize(obj.__dict__)
        self.assertTrue("a" in obj.__dict__.keys())
        self.assertEqual(obj.a, A_VALUE)
...
class TestSerializer(unittest.TestCase):
    ...
    def testConstructor(self):
        ...
        for obj in self.objs:
            serialization = rpickle.Serializer(obj)
            self.assertEqual(obj.__class__, serialization.cls)
            trues = [obj.__dict__[k] == serialization.obj_dct[k]
                  for k in serialization.obj_dct.keys()]
            self.assertTrue(all(trues))

    def testSerialize(self):
        if IGNORE_TEST:
            return
        def test(obj, hasSerializer):
            serialization = rpickle.Serializer(obj)
            serialization.serialize()
            values = [v for v in serialization.obj_dct.values()]
            isPresent = any([isinstance(v, rpickle.Serializer) for v in values])
            if hasSerializer:
